...
template <class T>
void IEulerIBVPFDM<T>::calculateBoundaryConditions(NumericMatrix<int, T>& mat, const NumericArray<int, T>& xArr, int t) {
    assert(t + 1 <= mat.maxRowIndex());
    mat(t + 1, mat.minColIndex()) = this->_ibvp->leftBC((t + 1) * this->_t_size, xArr[xArr.minIndex()]);
    mat(t + 1, mat.maxColIndex()) = this->_ibvp->rightBC((t + 1) * this->_t_size, xArr[xArr.maxIndex()]);
}
template <class T>
void IEulerIBVPFDM<T>::calculateSolution(NumericMatrix<int, T>& mat, const NumericArray<int, T>& xArr, int t) {
    ...
    for (int i = RHS.minIndex(); i <= RHS.maxIndex(); i++) {
        T sigma = this->_ibvp->diffusion((t + 1) * k, xArr[i+1]);
        T mew = this->_ibvp->convection((t + 1) * k, xArr[i+1]);
        double zero_term = this->_ibvp->zeroTerm((t + 1) * k, xArr[i+1]);
        
        A[i] = k * sigma - 0.5 * k * h * mew;
        B[i] = -1 * h * h - 2 * k * sigma + k * h * h * zero_term;
        C[i] = k * sigma + 0.5 * k * h * mew;

        RHS[i] = h * h * (k * this->_ibvp->rightHandSide((t+1) * k, xArr[i + 1]) - mat(t, i + 1));
    }
    
    RHS[0] = h * h * (k * this->_ibvp->rightHandSide((t + 1) * k, xArr[1]) - mat(t, 1)) - A[0] * mat(t + 1, 0);
    
    assert(RHS.maxIndex() == this->_x_steps - 2);
    assert(mat.maxColIndex() == this->_x_steps);
   
    RHS[RHS.maxIndex()] = h * h * (k * this->_ibvp->rightHandSide((t + 1) * k, xArr[xArr.maxIndex() - 1]) - mat(t, mat.maxColIndex() - 1)) - C[C.maxIndex()] * mat(t + 1, mat.maxColIndex());
